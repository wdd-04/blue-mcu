...
bit Seg_Disp_Mode;//0-参数 1-亮度等级
unsigned int Led_Time_Disp[4] = {400,400,400,400};
unsigned int Led_Time_Ctrol[4] = {400,400,400,400};
unsigned char Led_Time_Index;
unsigned int Timer_400Ms;
bit Seg_Star_Flag;
unsigned char Seg_Index;//0-熄灭 1-模式编号闪 2-流转间隔闪
...
void Seg_Proc()
{
	...
	Led_Level = Ad_Read(0x43) / 64;//亮度等级0 1 2 3 
	
	if(Seg_Disp_Mode == 0)//参数
	{	
		...
		Seg_Buf[7] = Led_Time_Disp[Led_Time_Index] / 1 % 10;		
		...
		else if(Seg_Index == 2)
		{
			...
			Seg_Buf[7] = Seg_Star_Flag?10:Led_Time_Disp[Led_Time_Index] / 1 % 10;		
		}
...
void Timer0Server() interrupt 1
{
	...
	//Seg_Slow_Down不改成10的话，长按s4，数码管还没来得及显示亮度等级就已经在显示界面了
